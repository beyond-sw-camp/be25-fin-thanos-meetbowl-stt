...
녹음 파일의 실제 저장 위치는 S3/MinIO 등 Object Storage를 사용한다.

| Method | Endpoint | 설명 | 호출 주체 |
|---|---|---|---|
| POST | `/sessions/{sessionId}/recording/start` | 녹음 시작 | meetbowl-be/System |
| POST | `/sessions/{sessionId}/recording/stop` | 녹음 종료 | meetbowl-be/System |
| GET | `/sessions/{sessionId}/recording` | 녹음 상태/메타데이터 조회 | meetbowl-be |
...
## 9. LiveKit DataChannel Producer
...
| `api.recording.save` | `recording.completed` | 녹음 파일 메타데이터 저장 요청 |
...
- 회의 녹음 파일은 Object Storage에 저장한다.
